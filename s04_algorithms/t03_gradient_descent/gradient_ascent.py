...
class GradientAscent(object):
    ...
    def sgd_run(self, X, y, alpha=0.01, step_size=0.0001):
        """
        INPUT: GradientAscent, 2 dimensional numpy array, numpy array
               float, int
        OUTPUT: None
        ...
        while diff >= step_size:
            ...
            for index in range(X.shape[0]):
                x_i = np.array(
                    [X[index, :]]
                )  # wrapped row in array( ) so that the same gradient and cost functions can be used
    ...
    def calculate_scaling_factors(self, X):
        if not self.scale:
            return
